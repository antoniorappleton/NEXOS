...
export const LoginView: React.FC<LoginViewProps> = ({ onBypass, onLoginSuccess, externalError }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        let finalRole = (typeof window !== 'undefined' ? localStorage.getItem('google_login_role') : null) || role || 'student';
        let finalName = userCred.user.displayName || 'Utilizador Google';
        let finalAvatar = userCred.user.photoURL || `https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=80&h=80&fit=crop&crop=face`;

        if (userCred.user.email) {
          const q = query(collection(db, 'profiles'), where('email', '==', userCred.user.email));
          ...
          if (!querySnap.empty) {
            ...
            if (querySnap.docs[0].id !== userCred.user.uid) {
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3 px-1">
                <span className="text-4xs text-slate-400 font-bold uppercase tracking-wider">Cargo para novas contas</span>
                <select
                  value={role}
                  onChange={(e: any) => setRole(e.target.value)}
                  className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-slate-300 text-4xs font-bold focus:outline-none focus:border-brand-500 transition cursor-pointer"
                >
                  <option value="student">Aluno</option>
                  <option value="teacher">Professor</option>
                  <option value="admin">Administrador</option>
